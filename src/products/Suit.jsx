...
const Suit = () => {
  ...
  const endpoint = useMemo(() => apiUrl('/api/products?category=suit'), []);
    ...
    const fetchProducts = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(payload.message || 'Failed to fetch suit collection');
        }
        ...
        if (isMounted) {
          setError(err.message || 'Unable to load suits');
        }
      } finally {
        if (isMounted) {
          setLoading(false);
  ...
  const formattedProducts = useMemo(
    () => products.map((item) => mapProductForDisplay({ ...item, category: item.category || 'Suit' })),
    [products]
  );
