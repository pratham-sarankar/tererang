            ...
            <div className="flex items-center justify-between">
                <div>
                    {product.displayOldPrice && (
                        <span className="line-through text-gray-400 text-base mr-2">{product.displayOldPrice}</span>
                    )}
...
const EthnicWear = () => {
    ...
    const endpoint = useMemo(() => apiUrl("/api/products?category=ethnicWear"), []);
    ...
    useEffect(() => {
        ...
        const fetchProducts = async () => {
            ...
            try {
                ...
                if (!res.ok) {
                    throw new Error("Failed to fetch winter ethnic collection");
                }
    ...
    const formattedProducts = useMemo(
        () => products.map((item) => mapProductForDisplay({ ...item, category: item.category || "Winter Ethnic Wear" })),
        [products]
    );
            ...
            <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white p-6 sm:p-10">
                <div className="text-center mb-16 pt-8">
                    <div className="border-4 border-black inline-block p-2 rounded-xl shadow-2xl">
                        <div className="bg-purple-700 p-6 rounded-lg text-white">
                            <p className="uppercase tracking-[0.5em] text-sm text-purple-200">Winter Edit</p>
                            <h1 className="text-4xl sm:text-5xl font-extrabold tracking-tight">Winter Ethnic Wear Collection</h1>
                            <p className="text-lg text-purple-100 mt-2">Cozy silhouettes with timeless embroidery — perfect for the season.</p>
                        </div>
                    </div>
                    <button
                        type="button"
                ...
                {loading && !error && (
                    <div className="flex justify-center py-20">
                        <span className="text-lg text-gray-500">Curating our winter ethnic edit...</span>
                    </div>
                )}

                {error && (
                    <div className="max-w-3xl mx-auto bg-white rounded-2xl shadow p-8 text-center border border-red-100">
                        <h2 className="text-2xl font-semibold text-red-500 mb-2">Unable to load collection</h2>
                ...
                {!error && !loading && formattedProducts.length === 0 && (
                    <div className="text-center text-gray-500 py-10">No items in this collection yet.</div>
                )}
