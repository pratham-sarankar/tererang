...
const Kurti = () => {
  ...
  const endpoint = useMemo(() => apiUrl("/api/products?category=kurti"), []);
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      ...
      try {
        ...
        if (!res.ok) {
          throw new Error("Failed to fetch kurti collection");
        }
  ...
  const formattedProducts = useMemo(
    () => products.map((item) => mapProductForDisplay({ ...item, category: item.category || "Kurti" })),
    [products]
  );
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white p-6 sm:p-10">
      <div className="text-center mb-16 pt-8">
        <div className="border-4 border-black inline-block p-2 rounded-xl shadow-2xl">
          <div className="bg-purple-700 p-6 rounded-lg text-white">
            <p className="uppercase tracking-[0.5em] text-sm text-purple-200">Signature Edit</p>
            <h1 className="text-4xl sm:text-5xl font-extrabold tracking-tight">The Tere Rang Kurti Collection</h1>
            <p className="text-lg text-purple-100 mt-2">
              Fluid silhouettes, modern embroidery, and everyday luxury tailored for you.
            </p>
          </div>
        </div>
        <button
      ...
      {error && (
        <div className="max-w-3xl mx-auto bg-white rounded-2xl shadow p-8 text-center border border-red-100">
          <h2 className="text-2xl font-semibold text-red-500 mb-2">Unable to load kurtis</h2>
