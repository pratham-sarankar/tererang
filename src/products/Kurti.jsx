...
// -------------------------------------------------------------------
// 3. PRODUCT LIST COMPONENT (Home Page) - UPDATED TO USE FETCHED DATA
// -------------------------------------------------------------------
...
                <p className="text-lg text-purple-200 mt-2">Discover the finest Kurti And Short Kurti .</p>
            </div>
        </div>
    </div>
...
      setError('');
      try {
        const res = await fetch(apiUrl('/api/products?category=kurti'));
...
  }, []);
  ...
  const dataset = data; // always use backend data; show empty state if none
...
  }
  ...
  if (dataset.length === 0) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center text-gray-700 p-6">
        <p className="mb-2 text-lg">No products found in Kurti category.</p>
